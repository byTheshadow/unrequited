...
function findLatestUnansweredUserChoiceInList(messagesList) {
...
function pickAutoQuoteTargetInList(messagesList) {
  const pool = messagesList.slice(-20).filter(isQuotableMsg);
  if (!pool.length) return null;
  return pool[Math.floor(Math.random() * pool.length)].id;
}

async function markLatestUserUnreadAsReadForConv(convId, messagesList, isCurrent) {
...
async function persistConvSummaryForConv(convId) {
  ...
  const unreadCount = await db.messages
    .where('conversationId')
    .equals(convId)
    .filter(m => !m.isRead && m.sender === 'character')
    .count();

  await db.conversations.update(convId, {
    lastMessage: last.content,
    lastMessageTime: last.timestamp,
    unreadCount: unreadCount // 写入未读计数字段，用于侧边栏红点渲染
  });
}
